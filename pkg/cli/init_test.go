...
func TestInit(t *testing.T) {
	...
	require.FileExists(t, path.Join(dir, "ssy.yaml"))
...
func TestInitSkipExisting(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(path.Join(dir, "ssy.yaml"), []byte("test123"), 0o644))
	...
	content, err := os.ReadFile(path.Join(dir, "ssy.yaml"))
	require.NoError(t, err)
