...
func TestFindProjectRootDirShouldFindParentDir(t *testing.T) {
	...
	err := os.WriteFile(path.Join(projectDir, "ssy.yaml"), []byte(testConfig), 0o644)
	...
	foundDir, err := findProjectRootDir(subdir, "ssy.yaml")
	require.NoError(t, err)
...
func TestFindProjectRootDirShouldReturnErrIfNoConfig(t *testing.T) {
	...
	_, err = findProjectRootDir(subdir, "ssy.yaml")
	require.Error(t, err)
}
